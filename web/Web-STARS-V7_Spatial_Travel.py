...
rk_map = us48_map.merge(usjoin, on='STATE_FIPS')
for year in years: rk_map[str(year)] = rankdata(df_map[str(year)], method='ordinal')
#print(rk_map)
...
app.layout = html.Div(
	html.Div([	
		...
		html.Div([
			html.H4('US PCR Data by state in year ',
            	id = 'choropleth-title',
...
def update_map_title(year_selected_slider):
    ...
    return 'US PCR Data by state in year {0}'.format(year)

############################################################

#@app.callback(
#	Output('example-graph', 'figure'),
#	[Input('years-slider','value'), Input('spatial_interval-event', 'n_intervals')],
#	[State('spatial_travel-check', 'values')])
#def update_map(year_selected_slider, n, checkedValues):
@app.callback(
	Output('example-graph', 'figure'),
	[Input('years-slider','value'), Input('spatial_interval-event', 'n_intervals')],
	[State('spatial_travel-check', 'values')],
...
def update_scatter_hovered(year_selected_slider):
...
@app.callback(
	...
	[Input('years-slider','value')])
def update_boxplot(year_selected_slider):
